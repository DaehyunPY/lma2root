	...
	if ((oei.GetNbrBytes() != fNbrBytes) || (saei.GetUsedMethod() != fMethod))
	{
		...
		if (saei.GetUsedMethod() == kCoM)
			if (oei.GetNbrBytes() == 1)
				...
				std::cout << "No Analysis Method for Pulses with "<<oei.GetNbrBytes()*8<<" Bit is available"<<std::endl;exit(0);}
			...
			fStdc = new MySoftTDCDoNothing();
				...
				std::cout << "Analysis Method "<<saei.GetUsedMethod()<<" not available"<<std::endl;exit(0);}
