...
TCanvas* Canv(int hor, int ver)
{
	...
	//setup canvas//
	TCanvas * MyC = new TCanvas("MyC","MyCanvas",-1,1,static_cast<int>(maxsize*hor/cw),static_cast<int>(maxsize*ver/cw));
				...
				for (size_t i=0;i<oc.GetNbrPulses();++i)
				{
					...
					if ((p.GetLength()>100)&&(chan!=(8-1)))
					{
						...
						for (int j=0; j<p.GetLength();++j)
						{
							...
							ChHistoCorr[chan]->SetBinContent(j+1+p.GetIndexToFirstPointOfOriginalWaveform(),(cData[j]-BLData[j]+offsetD)*vertGain);
						}
					}
...
{	
	...
	for (size_t i=0; i<oc.GetNbrPulses();++i)
	{
		...
		while (cont)
		{
			...
			//set the axis range//
			double minXAxis=timestamp; double maxXAxis=timestamp+xRange;
			double height = 0;
			...
			char out[256];
			if (Pol == kPositive)
				sprintf(out,"Sec:%s P:%s  D:%i F:%0.2f W:%0.2f T:%0.2f H:%0.1f",cs.GetName(),"Pos",static_cast<long>(delay*(oe.GetSampleInterval()*1e9)),fraction,walk,threshold,height);
			else if (Pol == kNegative)
				sprintf(out,"Sec:%s P:%s  D:%i F:%0.2f W:%0.2f T:%0.2f H:%0.1f",cs.GetName(),"Neg",static_cast<long>(delay*(oe.GetSampleInterval()*1e9)),fraction,walk,threshold,height);
			else
				sprintf(out,"Sec:%s P:%s  D:%i F:%0.2f W:%0.2f T:%0.2f H:%0.1f",cs.GetName(),"NO" ,static_cast<long>(delay*(oe.GetSampleInterval()*1e9)),fraction,walk,threshold,height);
			std::cout <<"                                                           \r"<<out;
