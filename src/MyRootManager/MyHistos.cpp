...
void MyHistos::ResetAll() {
  ...
  for (int i = 0; i < arsize; ++i) {
    ...
    if (obj) {
      ...
      TH1 *h = (TH1 *) obj;
      if (fVerbose)
        std::cout << "Resetting Histogram:" << h->GetName() << " Dir:" << h->GetDirectory()->GetName() << std::endl;
      //--reset histogram--//
      h->Reset();
    }
  }
...
TH1 *MyHistos::create3d(int id, const char *name,
  ...
  if (obj)
    std::cout << "name doesn't match(" << id << ") is:" << obj->GetName() << " should be:" << hist3->GetName()
              << std::endl;
...
void MyHistos::fill(int id, const char *name, double fillX, double fillY, double fillZ,
  ...
  else if ((!hist3->GetEntries())
      && ((nXbins != hist3->GetXaxis()->GetNbins()) || (xLow != hist3->GetXaxis()->GetXmin())
          || (xUp != hist3->GetXaxis()->GetXmax()) ||
          (nYbins != hist3->GetYaxis()->GetNbins()) || (yLow != hist3->GetYaxis()->GetXmin())
          || (yUp != hist3->GetYaxis()->GetXmax()) ||
          (nZbins != hist3->GetZaxis()->GetNbins()) || (zLow != hist3->GetZaxis()->GetXmin())
          || (zUp != hist3->GetZaxis()->GetXmax())))
...
TH1 *MyHistos::create2d(int id, const char *name,
  ...
  TObject *obj = hiarray[id];
  if (obj)
    std::cout << "name doesn't match(" << id << ") is:" << obj->GetName() << " should be:" << hist2->GetName()
              << std::endl;
...
void MyHistos::fill(int id, const char *name, double fillX, double fillY,
  ...
  if (!hist2) {
    ...
    //if the histogram exists, but has no entries, we want to resize and rebin it//
  else if ((!hist2->GetEntries())
      && ((nXbins != hist2->GetXaxis()->GetNbins()) || (xLow != hist2->GetXaxis()->GetXmin())
          || (xUp != hist2->GetXaxis()->GetXmax()) ||
          (nYbins != hist2->GetYaxis()->GetNbins()) || (yLow != hist2->GetYaxis()->GetXmin())
          || (yUp != hist2->GetYaxis()->GetXmax())))
...
TH1 *MyHistos::create1d(int id, const char *name,
  ...
  TObject *obj = hiarray[id];
  if (obj)
    std::cout << "name doesn't match(" << id << ") is:" << obj->GetName() << " should be:" << hist1->GetName()
              << std::endl;
...
void MyHistos::fill(int id, const char *name, double fillX, const char *titleX,
  ...
  if (!hist1) {
    ...
    //if the histogram exists, but has no entries, we want to resize and rebin it//
  else if ((!hist1->GetEntries())
      && ((nXbins != hist1->GetXaxis()->GetNbins()) || (xLow != hist1->GetXaxis()->GetXmin())
          || (xUp != hist1->GetXaxis()->GetXmax())))
