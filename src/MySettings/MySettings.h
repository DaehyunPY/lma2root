...
class MySetting {
 public:
  MySetting(const char *n, double v, const bool ve) :
      fName(n), fVal(v), fString(0), fVerbose(ve) {
    if (fVerbose)
      std::cout << "Created \"" << fName.Data() << "\" with " << fVal << std::endl;
  }
  MySetting(const char *n, const char *v, const bool ve) :
      fName(n), fVal(0), fString(v), fVerbose(ve) {
    if (fVerbose)
      std::cout << "Created \"" << fName.Data() << "\" with " << fString.Data() << std::endl;
  }
  ~MySetting() {}
...
//	MySetting(const MySetting& in)				{}
 ...
 public:
  ...
  void SetValue(double v) {
    fVal = v;
    std::cout << "Changing \"" << fName.Data() << "\" to " << fVal << std::endl;
  }
  void SetString(const char *v) {
    fString = v;
    std::cout << "Changing \"" << fName.Data() << "\" to " << fString.Data() << std::endl;
  }
...
class MySettings {
 public:
  ...
  ~MySettings() {/*std::cout <<"delete settings"<<std::endl;*/for (size_t i = 0; i < fSettings.size(); ++i)
      delete fSettings[i];/*std::cout<<"done"<<std::endl;*/}
...
//	MySettings(const MySettings& in)						{}
 ...
 private:
  ...
  MySetting *Add(const char *name, double val) {
    fSettings.push_back(new MySetting(name, val, fVerbose));
    return fSettings.back();
  }
  MySetting *Add(const char *name, const char *val) {
    fSettings.push_back(new MySetting(name, val, fVerbose));
    return fSettings.back();
  }
