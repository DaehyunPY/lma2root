...
bool MyFileProcessor::ProcessFile(const TString &fiName, MySettings &set, bool isFirstFile) {
  ...
  const size_t IntervalOfTerminalCounterUpdate = static_cast<size_t>(set.GetValue("IntervalOfTerminalCounterUpdate",
                                                                                  1)); //[2013/10/28 Matsunami] Interval for updating event counter which is displayed on terminal.
  ...
  if (fSAEI.ReadSettings(set, fOEI)) {
    if (fVerbose)
      std::cout
          << "something in the file header or settings for signal analyzed event has changed, copy changes from event header to event"
          << std::endl;
  ...
  if (fSEI.ReadSettings(set, fSAEI)) {
    if (fVerbose)
      std::cout << "something in the settings for sorted event has changed, copy changes from event header to event"
                << std::endl;
  ...
  while (!ar.EndOfFile()) {
    ...
    if (EventCounter % IntervalOfTerminalCounterUpdate == 0)
      std::cout << "\r" << "Event Counter :" << std::setw(5) << std::setfill(' ') << EventCounter + 1;
  ...
  std::cout << "Time (Reality):  " << time_reality << "s (" << static_cast<double>(EventCounter) / time_reality / 1000.
            << " KHz)" << std::endl;
  Double_t time_cpu = watch.CpuTime();
  std::cout << "Time (CPU):      " << time_cpu << "s (" << static_cast<double>(EventCounter) / time_cpu / 1000.
            << " KHz)" << std::endl;
