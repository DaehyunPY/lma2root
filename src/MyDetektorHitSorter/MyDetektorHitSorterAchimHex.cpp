...
MyDetektorHitSorterAchimHex::MyDetektorHitSorterAchimHex(const MyDetektorInfo &di, MyHistos &rm, int HiOff) :
  ...
  rm.create2d(fHiOff + kSumVsVShift,
              ...
              +10,
              Form("%s/Timesums", di.GetName()));
  rm.create2d(fHiOff + kSumVsWShift,
              "SumVsWShift",
              "w [ns]",
              "wsum [ns]",
              300,
  ...
  rm.create2d(fHiOff + kSumVsVShiftCorr,
              ...
              +10,
              Form("%s/Timesums", di.GetName()));
  rm.create2d(fHiOff + kSumVsWShiftCorr,
              "SumVsWShiftCorr",
              "w [ns]",
              "wsum [ns]",
              300,
  ...
  rm.create2d(fHiOff + kDetUVShi_ns,
              "DetUVShift_ns",
              "x [ns]",
              "y [ns]",
              300,
              ...
              maxpos_ns,
              Form("%s/DetPictures", di.GetName()));
  rm.create2d(fHiOff + kDetVWShi_ns,
              "DetVWShift_ns",
              "x [ns]",
              "y [ns]",
              300,
              ...
              maxpos_ns,
              Form("%s/DetPictures", di.GetName()));
  rm.create2d(fHiOff + kDetUWShi_ns,
              "DetUWShift_ns",
              "x [ns]",
              "y [ns]",
              300,
  ...
  rm.create2d(fHiOff + kDetShi_mm,
              ...
              maxpos_mm,
              Form("%s/DetPictures", di.GetName()));
  rm.create2d(fHiOff + kDetUVShi_mm,
              "DetUVShift_mm",
              "x [mm]",
              "y [mm]",
              300,
              ...
              maxpos_mm,
              Form("%s/DetPictures", di.GetName()));
  rm.create2d(fHiOff + kDetVWShi_mm,
              "DetVWShift_mm",
              "x [mm]",
              "y [mm]",
              300,
              ...
              maxpos_mm,
              Form("%s/DetPictures", di.GetName()));
  rm.create2d(fHiOff + kDetUWShi_mm,
              "DetUWShift_mm",
              "x [mm]",
              "y [mm]",
              300,
...
void MyDetektorHitSorterAchimHex::SortImpl(MySignalAnalyzedEvent &sae, MyDetektor &d, MyHistos &rm) {
  ...
  if (d.ActivateSorter() && (sae.GetChannel(7 - 1).GetNbrPeaks() < 1000)) {
    SortWithAchimSorter();
  } else
    fNRecHits = 0;
  ...
  if (fSwc) {
    ...
    if (fSwc->sumw_profile) {
      ...
      for (int binx = 0; binx < fSwc->sumw_profile->number_of_columns; ++binx) {
        file << di.GetName() << "_PosW" << std::setw(3) << binx << "="
             << fSwc->sumw_profile->get_bin_center_x(static_cast<double>(binx)) << "\t";
...
void MyDetektorHitSorterAchimHex::Shift(const MyDetektor &d) {
  ...
  fAs->shift_layer_w(+1,
                     d.GetWOffset());                                // shift layer w so that all center lines of the layers meet in one point
  fAs->shift_position_origin(-1,
                             d.GetDetCenterX(),
                             d.GetDetCenterY());    // shift all layers so that the position picture is centered around X=zero,Y=zero
}
