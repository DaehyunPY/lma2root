	...
	MyDetektorHitSorterQuad(di,rm,HiOff)
{
	...
	rm.create2d(fHiOff+kDetAll,"Det","x [mm]","y [mm]",200,-maxpos,maxpos,200,-maxpos,maxpos,Form("%s/SorterOutput",di.GetName()));
	rm.create1d(fHiOff+kTime,"Time","MCP [ns]",10000,0,maxtof,Form("%s/SorterOutput",di.GetName()));
	rm.create2d(fHiOff+kPosXVsTime,"PosXVsTime","MCP [ns]","x [mm]",500,0,maxtof,300,-maxpos,maxpos,Form("%s/SorterOutput",di.GetName()));
	rm.create2d(fHiOff+kPosYVsTime,"PosYVsTime","MCP [ns]","y [mm]",500,0,maxtof,300,-maxpos,maxpos,Form("%s/SorterOutput",di.GetName()));
	
	if (di.IsHexAnode()) std::cout << "using simple sorting for Hex-Anode.. Only U-Layer and V-Layer will be used!!!" <<std::endl;
}
...
void findBoundriesForSorting(const dVec& anode, const double mcp, const double ts, const double rTime, int &min, int &max)
{
	//--we know two things:--//
	//-- |x1-x2|<rTimex  and--//
			...
			for (int iX2=iX2min;iX2<=iX2max;++iX2)
			{
				...
				for (int iY1=iY1min;iY1<=iY1max;++iY1)
				{
					...
					for (int iY2=iY2min;iY2<=iY2max;++iY2)
					{
						...
						const double y_mm = (d.IsHexAnode())? 1./TMath::Sqrt(3.) * (x_mm - 2*((v1d[iY1] - v2d[iY2])*sfy)) : (v1d[iY1] - v2d[iY2])*sfy;
