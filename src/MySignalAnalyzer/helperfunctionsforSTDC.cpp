...
#include "../MyEvent/MySortedEvent/MyPunkt/MyPunkt.h"
...
void gewichtetlinearRegression(const int nbrPoints,
  ...
  for (int i = 0; i < nbrPoints; ++i) {
    double weight = (TMath::Abs(x[i] - correctX) > 1e-10) ? 1. / TMath::Abs(x[i] - correctX) : 100.;
    SumWeight += weight;
    SumX += (x[i] * weight);
    SumY += (y[i] * weight);
...
double findXForGivenY(const double *x, const double *coeff, const double Y, const double Start) {
  ...
  //liefere den dazugehigen x-Wert zur�Ek//
  if (p.y() == Y)
    return p.x();
  ...
  {
    if (p.y() > Y)        //ist der y-Wert grser als der gewollte
      ...
      return p.x();    //liefer den dazugehigen x-Wert zur�Ek
  } else        //wenn es ein positiver Druchgang ist
  {
    if (p.y() > Y)        //und der y-Wert grser als der gewollte
      Up = p;            //bildet der Punkt die neue obere Grenze
...
//_________________________________gib mir zurk______________________
