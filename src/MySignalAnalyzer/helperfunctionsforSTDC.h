...
void fwhm(const MyOriginalEvent &oe, const MyOriginalChannel &oc, MyPeak &p)
{
	...
	double lx[4];
	double ly[4];
	lx[0] = fwhm_l-2;	ly[0] = abs(Data[fwhm_l-2]-baseline);
	lx[1] = fwhm_l-1;	ly[1] = abs(Data[fwhm_l-1]-baseline);
	lx[2] = fwhm_l-0;	ly[2] = abs(Data[fwhm_l-0]-baseline);
	lx[3] = fwhm_l+1;	ly[3] = abs(Data[fwhm_l+1]-baseline);

	double rx[4];
	double ry[4];
	rx[0] = fwhm_r-1;	ry[0] = abs(Data[fwhm_r-1]-baseline);
	rx[1] = fwhm_r-0;	ry[1] = abs(Data[fwhm_r-0]-baseline);
	rx[2] = fwhm_r+1;	ry[2] = abs(Data[fwhm_r+1]-baseline);
	rx[3] = fwhm_r+2;	ry[3] = abs(Data[fwhm_r+2]-baseline);
