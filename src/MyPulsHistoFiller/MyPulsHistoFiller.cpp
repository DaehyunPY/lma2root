...
void extractNormPulsAndSlope(TH1 *p, TH1 *ps, const char *ChannelSectionName, ofstream &file) {
  ...
  for (int i = 1; i <= p->GetNbinsX(); ++i) {
    file << ChannelSectionName << "_MeanPuls" << std::setw(3) << std::setfill('0') << i << "="
         << (p->GetBinContent(i) / Maximum) << "\t";
...
double gmp(const double i, const MyOriginalEvent &e, const MyOriginalChannel &c, const MyPuls &pu, const MyPeak &p) {
  ...
  const double
      x0 = p.GetTime() / (e.GetSampleInterval() * 1e9) - pu.GetIndexToFirstPointOfOriginalWaveform() - e.GetHorpos();
...
template<typename T>
void fillMeanPulsAndFwhmHistos(MyHistos &hi,
                               const int off,
                               const MyOriginalEvent &e,
                               const MyOriginalChannel &c,
                               const MyPuls &pu,
                               const MyPeak &p) {
...
MyPulsHistoFillerBase::MyPulsHistoFillerBase(const MyOriginalEventInfo &oei,
                                             const MySignalAnalyzedEventInfo &saei,
                                             MyHistos &rm) {
    ...
    if (oei.GetUsedChannels() & (0x1 << ic)) {
      ...
      for (int ics = 0; ics < saci.GetNbrOfChannelSections(); ++ics) {
        ...
        if (!cs.IsOnlyVoltage()) {
          ...
          const char *dirnamePos = Form("Channel%02d/Section%02d(%d...%d)/Positive",
                                        ic + 1,
                                        ics + 1,
                                        cs.GetTimeRangeLow(),
                                        cs.GetTimeRangeHigh());
          const char *dirnameNeg = Form("Channel%02d/Section%02d(%d...%d)/Negative",
                                        ic + 1,
                                        ics + 1,
                                        cs.GetTimeRangeLow(),
                                        cs.GetTimeRangeHigh());
          ...
          const char *dirname =
              Form("Channel%02d/Section%02d(%d...%d)", ic + 1, ics + 1, cs.GetTimeRangeLow(), cs.GetTimeRangeHigh());
          rm.create1d(idxOffset,
                      "Voltage",
                      "U [mV]",
                      200,
                      -0.5 * oci.GetFullScale(),
                      0.5 * oci.GetFullScale(),
                      dirname);
        }
        idxOffset += 8;
...
void MyPulsHistoFiller8Bit::WritePulsHistosToFile(const MyOriginalEventInfo &oei,
                                                  const MySignalAnalyzedEventInfo &saei,
                                                  const MyHistos &rm) {
...
void MyPulsHistoFiller16Bit::WritePulsHistosToFile(const MyOriginalEventInfo &oei,
                                                   const MySignalAnalyzedEventInfo &saei,
                                                   const MyHistos &rm) {
  writeMeanPulsHistos(oei, saei, rm);
}
...
void MyPulsHistoFiller::Init(bool fill,
                             const MyOriginalEventInfo &oei,
                             const MySignalAnalyzedEventInfo &saei,
                             MyHistos &rm) {
